...
Read `.cursor/skills/implementation/task-pickup/SKILL.md` in full and follow its workflow: task lookup, conflict check,
...
Read `.cursor/skills/implementation/implement-plan/SKILL.md` in full and follow its workflow. Execute the plan in phases
...
just-completed phase. If it does, offer to run E2E tests via `.cursor/skills/implementation/run-e2e-plan/SKILL.md`
...
Read `.cursor/skills/pipelines/review-pipeline/SKILL.md` in full and follow its workflow.

## Phase 3: Ship (optional)

1. **Automated manual tests:** If the plan already has `### Phase N Manual Tests` sections, read
   `.cursor/skills/implementation/run-e2e-plan/SKILL.md` and run them. If the plan has no structured test sections, read
   `.cursor/skills/implementation/manual-test-plan/SKILL.md` to generate a test plan from the diff first, then run it
   via `run-e2e-plan`. Present results with screenshots and pause for user verification.
2. Read `.cursor/skills/implementation/babysit-pr/SKILL.md` — resolve comments, fix CI, resolve conflicts until
   merge-ready.
3. Read `.cursor/skills/implementation/acceptance-check/SKILL.md` — validate the diff against the task's acceptance
