...
- Related `docs/plans/` files if referenced
...
**Ordering strategy** (prefer this order within and across sections):

1. API schemas / contracts — the API contract
2. Database / data access layer — data access patterns
3. Server-side business logic — core backend logic
4. Client-side API consumption — how frontend consumes API
5. UI components — UI implementation
6. Tests (`.test.ts`, `.integration.test.ts`) — verify behavior
7. Docs/configs — reference material
...
- **Distrust the author's intent**: The code does what it does, not what the PR description says. If behavior diverges from stated intent, flag it.
- **Assume the worst timing**: Every async operation will interleave in the worst possible order. Every network call will timeout. Every shared state will be read stale.
- **Demand proof of correctness**: "It works" isn't enough. Can you trace the invariant through every code path? If not, it's a bug waiting to surface.
- **Hunt for what's missing**: The most dangerous bugs are the ones NOT in the diff — missing error handlers, absent cleanup, unprotected state transitions, untested branches.
- **Challenge pre-check assumptions**: Any "existing vs created" decision made before a write may be stale by cleanup time. Any guard that reads then writes without atomicity is a TOCTOU bug.
- **Reject cargo-culting**: If code copies a pattern from elsewhere without understanding it, call it out. Patterns applied without thought are tech debt.

### Scrutiny Criteria

Apply your project's review checks (see `checks.md` in this directory for examples). Only report findings that are relevant — don't force every
...
- Use the documentation-update skill decision matrix to identify which doc types are
  affected by the changes (README indexes, `docs/*.md`, `.d2` diagrams, `AGENTS.md`, `CLAUDE.md`, etc.)
- List any existing docs that should be updated based on the changes (reference the Documentation Impact table in
  `checks.md`)
- **Agent / model changes:** If the PR changes which models agents use (defaults, routing, fallbacks), embeddings,
  (pricing table, scenario estimates, code pointers) so operating-cost reference stays accurate
- **Feature flags:** If the PR adds, removes, or changes a `FeatureFlagName` enum value, flag whether
  new flag's description, gated surfaces, and suggested GA date
- Note any new behavior that lacks documentation coverage
- If the PR adds a new `docs/` file, flag whether a new row should be added to the doc impact table in `checks.md`
...
- If the diff touches agent instructions/prompts alongside eval fixtures or scorers, recommend running the

---

## Review Principles

- **Assume bugs exist** — your job is to find them. Every line is suspicious until you've convinced yourself it's correct.
- **No benefit of the doubt** — if something looks fragile, it IS fragile. Don't rationalize it away.
- **Challenge every non-trivial decision** — "why this approach and not a simpler one?" If the author can't justify the complexity, it shouldn't be there.
- **Think like a hostile environment** — concurrent requests, partial failures, stale state, malicious input, unexpected ordering. If the code doesn't defend against it, call it out.
- **Demand elegance ruthlessly** — if a fix feels hacky, reject it. There is almost always a cleaner way. The only exception is a one-line obvious fix where over-engineering would be worse.
- **Distinguish severity** — blockers vs. nitpicks matter, but when in doubt, escalate severity. A "minor" issue that ships is a production bug.
- **Be specific and actionable** — vague concerns are useless. Show the failure scenario, name the race condition, cite the line.
