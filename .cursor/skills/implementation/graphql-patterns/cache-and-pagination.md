...
- A domain-specific plural field: `{pluralTypeName}: [Type!]!` (e.g., `tasks: [Task!]!`, `feedbacks: [Feedback!]!`, `projects: [Project!]!`). Use generic `items: [Type!]!` only for wrapper types that don't correspond to a single domain entity (e.g., `CompanyWithCountConnection`).
- `hasMore: Boolean!`
- `total: Int!`
