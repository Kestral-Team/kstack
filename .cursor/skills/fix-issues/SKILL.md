...
- **Rollback ownership (TOCTOU guard)**: destructive cleanup must use authoritative write-path ownership signals (for example `wasCreated`), never stale pre-check state.
